...
type t = 
{
  map: (string, string) Hashtbl.t;
  counter: int ref
}

let init = { map = Hashtbl.create 32; counter = ref 0 }

let copy { map; counter } = { map = Hashtbl.copy map; counter = counter }

let rec resolve program =
  let state = init in
  resolve_program program state

and resolve_program (program: Vir.program) (state: t) : (Vir.program) =
...
| Vir.LabeledStmt { stmt; label } -> 
  ...
  Vir.LabeledStmt { stmt = gather_statement stmt state; label = unique_name }
...
| Vir.Switch { expr; stmt; label; cases; default } -> 
  Vir.Switch { 
    expr; 
    stmt = gather_statement stmt state; 
    label;  cases;  default 
  }
...
| Vir.Goto target -> 
  ...
  | None -> failwith (Printf.sprintf "label with name %s is not defined" target))
