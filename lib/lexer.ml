...
let init input =
  {input; ch = if (String.length input) = 0 then None else Some (String.get input 0); pos = 0} 
...
and next_token lexer = 
  ...
  | Some ch ->
    ...
    let peek_are ch1 ch2 = peek_char lexer = Some ch1 && peek_next_char lexer = Some ch2 in
    let lexer, token =
    match ch with
    | '(' -> sym1 LParen
    | ')' -> sym1 RParen
...
and read_while lexer pred =
  ...
  let lexer, end_pos = seek lexer (fun ch ->
    match ch with
    | Some char -> pred char
    | None -> false) 
  in
