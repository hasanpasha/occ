...
let rec lower (program: Vir.t) : t =
  List.map (fun decl : function' -> match decl with
  | Vir.Function { name; body } ->
    let counter = ref 0 in 
    let instructions = lower_block body counter in
    { name; instructions }

  | Vir.Variable _ -> failwith "top level variable declaration are not supported yet") program
...
and lower_declaration decl counter =
  ...
  | Vir.Variable { name; init } -> 
    ...
    | Some expr -> 
      ...
      instrs @ [ Copy { src = value; dst = Variable (make_unique name counter) } ]
    | None -> [])
| Vir.Function _ -> []
...
| Vir.Switch { expr; stmt; label; cases; default } -> 
  ...
  let value, instrs = lower_expression expr counter in
  instrs
  @ List.concat (List.map (fun (expr, label) -> 
    let constant_value, _ = lower_expression expr counter in
    let cond = make_temp_var counter in
    [
      Binary { operator = Equals; src1 = value; src2 = constant_value; dst = cond };
...
| Vir.Case { expr = _; stmt; label } | Vir.Default {  stmt; label } -> 
  [Label label]
  @ (match stmt with 
  | Some stmt -> lower_statement stmt counter
  | None -> [])

and lower_expression (expr: Vir.expression) (counter: int ref) : (value * instruction list) =
match expr with
| Vir.IntLit value -> Constant value, []
...
| Vir.LeftUnary { operator = PlusPlus | MinusMinus as operator; rhs } ->
  ...
  dst, src_instrs
  @ [
    Binary { operator; src1 = src_value; src2 = Constant 1; dst };
...
| Vir.RightUnary { operator = PlusPlus | MinusMinus as operator; lhs } ->
  ...
  @ [
    Copy { src; dst };
    Binary { operator; src1 = src; src2 = Constant 1; dst };
  ]
